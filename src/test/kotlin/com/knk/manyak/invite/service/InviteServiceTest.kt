 ...
 *   여기서는 올바른 멱등 키·상한·집계 구간 전달까지만 검증하고, 상한 스킵 자체는 지갑 서비스 통합 테스트가 검증한다.
 *
...
class InviteServiceTest {
    ...
    fun `초대 조회는 이번 KST 월 초대 보상 건수와 월 상한을 함께 반환한다`() {
        ...
        `when`(
            creditWalletService.countRewardsInWindow(7L, CreditReason.INVITE_REWARD, julyStart, augustStart),
        ).thenReturn(3L)
    ...
    fun `redeem은 정규화된 코드로 초대자를 찾아 관계를 저장하고 양쪽에 적립을 위임한다`() {
        ...
        val expectedCap = MonthlyRewardCap(CreditReason.INVITE_REWARD, inviteMonthlyCap, julyStart, augustStart)
        ...
        // 월 상한은 CreditWalletService.reward가 지갑 락 안에서 판정하도록 MonthlyRewardCap으로 위임한다.
        ...
        assertThat(redeemerReward.getArgument<MonthlyRewardCap>(6)).isEqualTo(expectedCap)
    }
    ...
    fun `redeem의 월 상한 집계 구간은 적립 시점의 KST 월이다`() {
        ...
        val expectedCap = MonthlyRewardCap(CreditReason.INVITE_REWARD, inviteMonthlyCap, augustStart, kstMonthStart(2026, 9))
        assertThat(rewardInvocations()).allSatisfy {
            assertThat(it.getArgument<MonthlyRewardCap>(6)).isEqualTo(expectedCap)
        }
    }

    @Test
    fun `제출자가 월 상한으로 스킵되면 amount는 0이고 요청은 성공한다`() {
        val redeemer = User(id = 9L, nickname = "제출자")
        val inviter = User(id = 5L, nickname = "초대자", inviteCode = "GOOD5555")
        `when`(userRepository.findByIdForUpdate(9L)).thenReturn(redeemer)
        `when`(userRepository.findByInviteCode("GOOD5555")).thenReturn(inviter)
