...
/**
 * SocialLoginService의 오케스트레이션 계약을 고정한다(검증 → find-or-create → 토큰 발급).
 *
 * 흐름은 provider와 무관하게 동일하므로(스펙 §4-5) 대부분은 Google로 고정해 보고,
 * provider가 실제로 아래까지 전달되는지는 Kakao 케이스로 확인한다.
 *
 * 영속성은 [SocialAccountRegistrar] mock으로 대체하고(트랜잭션 경계는 그 빈이 가진다),
 ...
 * - 검증기가 없는 provider(APPLE·NAVER 예약): 토큰을 보지도 않고 401(fail-closed).
 *
...
class SocialLoginServiceTest {
    ...
    private val registrar: SocialAccountRegistrar = mock(SocialAccountRegistrar::class.java)
    ...
    private fun fakeVerifier(providerUserId: String = "sub"): SocialIdTokenVerifier =
        SocialIdTokenVerifier { SocialUserInfo(providerUserId = providerUserId) }
    ...
    private fun anyProvider(): SocialProvider = any(SocialProvider::class.java) ?: SocialProvider.GOOGLE
    ...
    fun `기존 사용자면 재사용해 토큰을 발급하고 새로 만들지 않는다`() {
        ...
        `when`(registrar.findExistingUser(anyProvider(), anySocialUserInfo(), anyInstant())).thenReturn(user)

        serviceWith(fakeVerifier("sub")).login(SocialProvider.GOOGLE, "dummy")

        verify(registrar, never()).createUserAndAccount(anyProvider(), anySocialUserInfo(), anyInstant())
    ...
    fun `신규 사용자면 생성해 토큰을 발급한다`() {
        ...
        `when`(registrar.findExistingUser(anyProvider(), anySocialUserInfo(), anyInstant())).thenReturn(null)
        `when`(registrar.createUserAndAccount(anyProvider(), anySocialUserInfo(), anyInstant())).thenReturn(created)

        serviceWith(fakeVerifier("sub")).login(SocialProvider.GOOGLE, "dummy")

        verify(registrar).createUserAndAccount(anyProvider(), anySocialUserInfo(), anyInstant())
        verify(authTokenService).issueTokens(created)
    }

    // ---- provider 전달·격리(스펙 §4-5, KNK-727) ----

    @Test
    fun `카카오 로그인은 KAKAO provider로 조회하고 생성한다`() {
        val created = User(id = 9L, nickname = "카카오신규")
        `when`(registrar.findExistingUser(anyProvider(), anySocialUserInfo(), anyInstant())).thenReturn(null)
        `when`(registrar.createUserAndAccount(anyProvider(), anySocialUserInfo(), anyInstant())).thenReturn(created)

        serviceWith(fakeVerifier("kakao-sub")).login(SocialProvider.KAKAO, "dummy")

        // 요청 경로(엔드포인트)가 정한 provider가 영속 계층까지 그대로 전달돼야 한다(계정 통합 없음 — §4-5).
        val find = mockingDetails(registrar).invocations.first { it.method.name == "findExistingUser" }
        assertThat(find.getArgument<SocialProvider>(0)).isEqualTo(SocialProvider.KAKAO)
        val create = mockingDetails(registrar).invocations.single { it.method.name == "createUserAndAccount" }
        assertThat(create.getArgument<SocialProvider>(0)).isEqualTo(SocialProvider.KAKAO)
        verify(authTokenService).issueTokens(created)
    }

    @Test
    fun `검증기가 없는 provider면 토큰을 보지 않고 401이다`() {
        // APPLE·NAVER는 enum에만 예약돼 있다(스펙 §4-5). 배선이 없는 provider가 새면 검증 없는 로그인이 열리므로 fail-closed로 막는다.
        assertThatThrownBy { serviceWith(fakeVerifier("sub")).login(SocialProvider.APPLE, "dummy") }
    ...
    // ---- 분석 이벤트 배선(스펙 §6-4-2-8) ----

    @Test
    fun `성공 이벤트는 로그인에 쓰인 provider로 발행한다`() {
        val created = User(id = 9L, nickname = "카카오신규")
        `when`(registrar.findExistingUser(anyProvider(), anySocialUserInfo(), anyInstant())).thenReturn(null)
        `when`(registrar.createUserAndAccount(anyProvider(), anySocialUserInfo(), anyInstant())).thenReturn(created)
        ...
        val emitted = mockingDetails(serverAnalytics).invocations.single { it.method.name == "socialLoginSucceeded" }
        assertThat(emitted.getArgument<SocialProvider>(0)).isEqualTo(SocialProvider.KAKAO)
        assertThat(emitted.getArgument<Boolean>(2)).isTrue()
    }

    @Test
    fun `검증 실패 이벤트도 해당 provider로 발행한다`() {
        val verifier = SocialIdTokenVerifier {
            throw ResponseStatusException(HttpStatus.UNAUTHORIZED, "유효하지 않은 Kakao ID 토큰입니다.")
        }

        assertThatThrownBy { serviceWith(verifier).login(SocialProvider.KAKAO, "bad-token") }
            .isInstanceOf(ResponseStatusException::class.java)

        val emitted = mockingDetails(serverAnalytics).invocations.single { it.method.name == "socialLoginFailed" }
        assertThat(emitted.getArgument<SocialProvider>(0)).isEqualTo(SocialProvider.KAKAO)
    }
    ...
    fun `미스냅샷 계정은 체험을 스냅샷하고 완료를 기록한다`() {
        ...
        serviceWith(fakeVerifier("sub")).login(SocialProvider.GOOGLE, "dummy", deviceId = "dev-1")
    ...
    fun `핸드오프의 원본 디바이스 ID가 요청 헤더보다 우선해 시드된다`() {
        ...
        serviceWith(fakeVerifier("sub"))
            .login(SocialProvider.GOOGLE, "dummy", deviceId = EXTERNAL_DEVICE, handoffCode = "code")
    ...
    fun `무효한 핸드오프 코드는 로그인을 막지 않고 헤더 디바이스로 폴백한다`() {
        ...
        serviceWith(fakeVerifier("sub"))
            .login(SocialProvider.GOOGLE, "dummy", deviceId = EXTERNAL_DEVICE, handoffCode = "expired")
    ...
    fun `소비가 실패해도 로그인은 성공한다`() {
        ...
        val tokens = serviceWith(fakeVerifier("sub"))
            .login(SocialProvider.GOOGLE, "dummy", deviceId = IN_APP_DEVICE, handoffCode = "code")
    ...
    fun `동시 첫 로그인 재조회로 재사용한 User에도 멱등 키로 보상을 적립한다`() {
        ...
        `when`(registrar.findExistingUser(anyProvider(), anySocialUserInfo(), anyInstant()))
            .thenReturn(null)
            .thenReturn(concurrentlyCreated)
        `when`(registrar.createUserAndAccount(anyProvider(), anySocialUserInfo(), anyInstant()))
    ...
    fun `동시 첫 로그인으로 create가 유니크 위반이면 재조회로 기존 User를 재사용한다`() {
        ...
        verify(registrar, times(2)).findExistingUser(anyProvider(), anySocialUserInfo(), anyInstant())
    ...
    fun `create가 유니크 위반인데 재조회로도 못 찾으면 원 예외를 던진다`() {
        ...
        assertThatThrownBy { serviceWith(fakeVerifier("sub")).login(SocialProvider.GOOGLE, "dummy") }
    ...
    fun `검증 실패면 401을 전파하고 저장 부작용이 없다`() {
        ...
        assertThatThrownBy { serviceWith(verifier).login(SocialProvider.GOOGLE, "bad-token") }
            .isInstanceOf(ResponseStatusException::class.java)
            .extracting("statusCode")
            .hasToString("401 UNAUTHORIZED")
