...
/**
 * 익명 ID 해시 규칙을 고정한다: 고정(결정적) 해시 + anon_hash_ 접두 + 원본 미노출.
 * 분석에서 동일 사용자를 묶으려면 같은 입력이 항상 같은 해시여야 하고,
 * 개인정보 원칙(AN-3 §8)상 원본 익명 ID는 로그/해시 결과에 노출되면 안 된다.
 */
class AnonymousIdHasherTests {

    private val hasher = AnonymousIdHasher(pepper = "")
    ...
    @Test
    fun `해시는 anon_hash_ 접두와 고정 길이를 가진다`() {
        val hash = hasher.hash("anon-12345")
        assertThat(hash).startsWith("anon_hash_")
        assertThat(hash).hasSize("anon_hash_".length + 16)
    }
    ...
    fun `pepper가 다르면 같은 입력도 다른 해시가 된다`() {
        val raw = "anon-12345"
        assertThat(AnonymousIdHasher(pepper = "p1").hash(raw))
            .isNotEqualTo(AnonymousIdHasher(pepper = "p2").hash(raw))
    }
}
