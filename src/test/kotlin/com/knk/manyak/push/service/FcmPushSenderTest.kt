...
class FcmPushSenderTest {
    ...
    private val sender = FcmPushSender(messaging, repository, registry)
    ...
    fun `서비스 계정이 없으면 아무것도 하지 않는다`() {
        val disabled = FcmPushSender(null, repository, registry)

        disabled.sendToUser(7L, mapOf("type" to "STORY_COMPLETED"))

        verify(repository, never()).findAllByUserId(7L)
        verify(messaging, never()).send(any())
    }

    @Test
    fun `등록된 기기마다 한 건씩 보낸다`() {
        `when`(repository.findAllByUserId(7L)).thenReturn(listOf(token(1, "tok-a"), token(2, "tok-b")))
    ...
    fun `토큰이 없으면 발송하지 않는다`() {
        `when`(repository.findAllByUserId(7L)).thenReturn(emptyList())
    ...
    fun `UNREGISTERED면 그 토큰만 지우고 나머지는 계속 보낸다`() {
        ...
        `when`(repository.findAllByUserId(7L)).thenReturn(listOf(dead, alive))
    ...
    fun `다른 FCM 오류는 토큰을 남기고 실패로 센다`() {
        `when`(repository.findAllByUserId(7L)).thenReturn(listOf(token(1, "tok-a")))
