 ...
 *
 * MDC에는 원본 익명 ID가 아닌 해시(anonymous_id_hash)만 존재하므로, 원본 PII는 구조적으로 forward되지 않는다(§8).
 ...
 * 익명 ID는 원본 헤더(X-Manyak-Anonymous-Id)와 구분되는 해시 전용 헤더로 내보낸다.
 */
object CorrelationHeaders {
    ...
    const val HEADER_ANONYMOUS_ID_HASH = "X-Manyak-Anonymous-Id-Hash"
    ...
    fun forwardingHeadersFromMdc(): Map<String, String> = buildMap {
        ...
        putIfPresent(HEADER_ANONYMOUS_ID_HASH, MDC.get(MdcKeys.ANONYMOUS_ID_HASH))
    }
