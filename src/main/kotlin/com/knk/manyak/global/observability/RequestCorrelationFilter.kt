...
/**
 * 모든 요청에서 X-Manyak-* 식별자 헤더를 받아 MDC(request_id/session_id/anonymous_id_hash)에 적재하고
 ...
 *
 * 필수 헤더(anonymous/session)가 없어도 요청을 막지 않는다. 분석 신호 누락이 제품 가용성을 깨면
 * 안 되므로 unknown으로 채우고 경고만 남긴다. request_id는 없으면 생성한다.
 */
@Component
@Order(Ordered.HIGHEST_PRECEDENCE)
class RequestCorrelationFilter(
    private val anonymousIdHasher: AnonymousIdHasher,
    ...
    override fun doFilterInternal(
        ...
        val rawAnonymousId = request.headerOrNull(HEADER_ANONYMOUS_ID)

        warnIfRequiredHeadersMissing(request, rawSessionId, rawAnonymousId)

        response.setHeader(HEADER_REQUEST_ID, requestId)
        MDC.put(MdcKeys.REQUEST_ID, requestId)
        MDC.put(MdcKeys.SESSION_ID, rawSessionId ?: UNKNOWN)
        MDC.put(MdcKeys.ANONYMOUS_ID_HASH, rawAnonymousId?.let(anonymousIdHasher::hash) ?: UNKNOWN)
        try {
    ...
    private fun warnIfRequiredHeadersMissing(
        ...
        val missing = buildList {
            if (rawAnonymousId == null) add(HEADER_ANONYMOUS_ID)
    ...
    companion object {
        const val HEADER_ANONYMOUS_ID = "X-Manyak-Anonymous-Id"
